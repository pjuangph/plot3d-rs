...
fn debug_block_2118() {
    ...
    println!(
        ...
        TARGET_BLOCK, b.imax, b.jmax, b.kmax
    );
    ...
    for f in &my_outer {
        ...
        if !found_any {
            ...
            println!(
                "    theta range: {:.4} to {:.4} rad ({:.2} to {:.2} deg)",
                theta1,
                theta2,
                theta1.to_degrees(),
                theta2.to_degrees()
    ...
    let my_remaining: Vec<&FaceRecord> = remaining
        ...
        .collect();
    println!(
        "\nRemaining non-connected faces for block {} after periodicity:",
        TARGET_BLOCK
    );
    ...
    for (idx, ((i, j, k), coord)) in corners.iter().zip(corner_coords.iter()).enumerate() {
        println!(
            "    [{}] ({},{},{}) -> ({:.6}, {:.6}, {:.6})",
            idx, i, j, k, coord[0], coord[1], coord[2]
        );
    }

    let search_tol = 1e-4;
    let mut matching_blocks: std::collections::HashMap<usize, Vec<String>> =
    ...
    for (ci, coord) in corner_coords.iter().enumerate() {
        for (bi, blk) in blocks.iter().enumerate() {
            ...
            // Quick AABB check
            let (xn, xx, yn, yx, zn, zx) = block_aabb(blk);
            if coord[0] < xn - search_tol
                || coord[0] > xx + search_tol
                || coord[1] < yn - search_tol
                || coord[1] > yx + search_tol
                || coord[2] < zn - search_tol
                || coord[2] > zx + search_tol
            {
                continue;
            }
            ...
            for &(i, j, k) in &boundary_indices {
                ...
                let dist =
                    ((bx - coord[0]).powi(2) + (by - coord[1]).powi(2) + (bz - coord[2]).powi(2))
                        .sqrt();
                if dist < search_tol {
                    matching_blocks.entry(bi).or_default().push(format!(
                        "corner[{}] matched ({},{},{}) dist={:.2e}",
                        ci, i, j, k, dist
                    ));
    ...
    if matching_blocks.is_empty() {
        println!(
            "  NO blocks share any boundary nodes with K=24 corners - this is a genuine boundary"
        );
    } else {
        println!(
            "  {} blocks share boundary nodes with K=24 corners:",
            matching_blocks.len()
        );
        for (bi, msgs) in &matching_blocks {
            println!(
                "    Block {} (dims {}x{}x{}):",
                bi, blocks[*bi].imax, blocks[*bi].jmax, blocks[*bi].kmax
            );
            for msg in msgs {
                println!("      {}", msg);
            }
    ...
    for direction in &["forward", "backward"] {
        let angle = if *direction == "forward" {
            rot_angle
        } else {
            -rot_angle
        };
        ...
        for (ci, coord) in corner_coords.iter().enumerate() {
            ...
            let rotated = [
                coord[0],
                coord[1] * cos_a - coord[2] * sin_a,
                coord[1] * sin_a + coord[2] * cos_a,
            ];
            ...
            for (bi, blk) in blocks.iter().enumerate() {
                ...
                let (xn, xx, yn, yx, zn, zx) = block_aabb(blk);
                if rotated[0] < xn - search_tol
                    || rotated[0] > xx + search_tol
                    || rotated[1] < yn - search_tol
                    || rotated[1] > yx + search_tol
                    || rotated[2] < zn - search_tol
                    || rotated[2] > zx + search_tol
                {
                    continue;
                }
            ...
            if !found {
                println!(
                    "    corner[{}] rotated({:.6},{:.6},{:.6}) - no match found",
    ...
    println!(
        "  Block 2386 dims: {}x{}x{}",
        b2386.imax, b2386.jmax, b2386.kmax
    );
    ...
    if let Some(k0) = k0_face {
        ...
        let mut target_k24 =
            create_face_from_diagonals(&blocks[TARGET_BLOCK], 0, 0, 24, 144, 48, 24);
        target_k24.set_block_index(TARGET_BLOCK);

        let (pts, _, _) =
            get_face_intersection(&target_k24, k0, &blocks[TARGET_BLOCK], b2386, 1e-6);
        println!(
            "  get_face_intersection result: {} matched points",
            pts.len()
        );
        for pt in pts.iter().take(5) {
            ...
            println!(
                "    ({},{},{}) <-> ({},{},{})  dist={:.2e}",
                pt.i1, pt.j1, pt.k1, pt.i2, pt.j2, pt.k2, dist
            );
        }
    }
    ...
    println!(
        "  Block 2388 dims: {}x{}x{}",
        b2388.imax, b2388.jmax, b2388.kmax
    );
    ...
    if let Some(j0) = j0_face {
        ...
        let (pts, _, _) =
            get_face_intersection(&target_k24, j0, &blocks[TARGET_BLOCK], b2388, 1e-6);
        println!(
            "  get_face_intersection result: {} matched points",
            pts.len()
        );
    ...
    println!(
        "\n  Block 2386 in overlapping list: {}",
        overlapping.contains(&2386)
    );
    println!(
        "  Block 2388 in overlapping list: {}",
        overlapping.contains(&2388)
    );

    // Check AABB of blocks 2386 and 2388
    let (xn, xx, yn, yx, zn, zx) = block_aabb(b2386);
    println!(
        "  Block 2386 AABB: x=[{:.6},{:.6}], y=[{:.6},{:.6}], z=[{:.6},{:.6}]",
        xn, xx, yn, yx, zn, zx
    );
    let (xn, xx, yn, yx, zn, zx) = block_aabb(b2388);
    println!(
        "  Block 2388 AABB: x=[{:.6},{:.6}], y=[{:.6},{:.6}], z=[{:.6},{:.6}]",
        xn, xx, yn, yx, zn, zx
    );
    ...
    println!(
        "  Block 2118 reduced: {}x{}x{}",
        reduced[TARGET_BLOCK].imax, reduced[TARGET_BLOCK].jmax, reduced[TARGET_BLOCK].kmax
    );
    println!(
        "  Block 2386 reduced: {}x{}x{}",
        reduced[2386].imax, reduced[2386].jmax, reduced[2386].kmax
    );
    println!(
        "  Block 2388 reduced: {}x{}x{}",
        reduced[2388].imax, reduced[2388].jmax, reduced[2388].kmax
    );
    ...
    println!(
        "  Reduced block 2118 AABB: x=[{:.6},{:.6}], y=[{:.6},{:.6}], z=[{:.6},{:.6}]",
        xn1, xx1, yn1, yx1, zn1, zx1
    );
    println!(
        "  Reduced block 2386 AABB: x=[{:.6},{:.6}], y=[{:.6},{:.6}], z=[{:.6},{:.6}]",
        xn2, xx2, yn2, yx2, zn2, zx2
    );
    println!(
        "  Reduced block 2388 AABB: x=[{:.6},{:.6}], y=[{:.6},{:.6}], z=[{:.6},{:.6}]",
        xn3, xx3, yn3, yx3, zn3, zx3
    );

    let tol = 1e-6;
    let overlap_2386 = xx1 + tol >= xn2
        && xx2 + tol >= xn1
        && yx1 + tol >= yn2
        && yx2 + tol >= yn1
        && zx1 + tol >= zn2
        && zx2 + tol >= zn1;
    let overlap_2388 = xx1 + tol >= xn3
        && xx3 + tol >= xn1
        && yx1 + tol >= yn3
        && yx3 + tol >= yn1
        && zx1 + tol >= zn3
    ...
    let mut target_face_reduced = create_face_from_diagonals(
        ...
        0,
        k_max_reduced,
        reduced[TARGET_BLOCK].imax - 1,
        reduced[TARGET_BLOCK].jmax - 1,
        k_max_reduced,
    );
    target_face_reduced.set_block_index(TARGET_BLOCK);
    println!(
        "  Target face (reduced): i={}..{}, j={}..{}, k={}",
        target_face_reduced.imin(),
        target_face_reduced.imax(),
        target_face_reduced.jmin(),
        target_face_reduced.jmax(),
        k_max_reduced
    );
